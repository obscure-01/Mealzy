...
export async function createOrderItems(client, order_item_positions, order_item_values) {
    const sql = `INSERT INTO order_items (order_id, item_id, quantity, price_at_order) VALUES ${order_item_positions.join(", ")}`
...
export async function getMultipleOrdersItems(positions, values) {
    const sql = `SELECT o.order_id, i.item_name, i.image_url, i.category, i.is_vegetarian, o.quantity, o.price_at_order FROM order_items o JOIN items i on o.item_id = i.item_id WHERE o.order_id IN (${positions.join(', ')})`;
...
export async function getCanteenOrderHistory(canteen_id) {
    const sql = "SELECT u.name, u.phone_number, u.email, u.profile_picture, o.order_id, o.status, o.accepted_by, o.order_time, o.completed_at, o.cancelled_at, o.estimated_ready_time FROM orders o JOIN users u ON o.user_id = u.user_id WHERE o.canteen_id = $1 ORDER BY o.status, o.order_time;"
